...
{
    short Event = tools_byte(&buf[0]);
#if 0
    short EventMinor = Event == LBXEvent ? tools_byte(&buf[1]) : 0;
...
{
    short Request = tools_byte(&buf[0]);
    short RequestMinor = Request >= 128 ? tools_byte(&buf[1]) : 0;
    unsigned long seq;
#if 0
